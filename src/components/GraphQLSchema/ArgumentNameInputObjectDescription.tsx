...
function ArgumentNameInputObjectDescription() {
  const { argNameDescrInputObject: argNameDescrInputObjectParams } = useParams();
  ...
  const field = schemaValue.find((fieldItem) => fieldItem.name === argInputObjectParams);
  if (!field) {
    return <Loader />;
  }

  const fieldsArgsNames = field.inputFields;
  if (!fieldsArgsNames.length) {
    return <Loader />;
  }
  const fieldsArgName = fieldsArgsNames.find(
    (fieldsArgItem) => fieldsArgItem.name === argNameDescrInputObjectParams
  );
  ...
  return (
    <>
      <div className="graphiql_schema__header">
        ...
        <h3 className="graphiql_schema__header_h3">{argNameDescrInputObjectParams}</h3>
      </div>
...
export default ArgumentNameInputObjectDescription;
